...
STOCKS_LIST: Final[list[str]] = [
    "NVDA - NVIDIA - May 20, 2026 - After Close",
    "MU - Micron Technology - ~Jun 24, 2026 - After Close",
    "ASML - ASML - ~Jul 15, 2026 - Before Open",
    "TSLA - Tesla - ~Jul 22, 2026 - After Close",
    "GOOGL - Alphabet - ~Jul 22, 2026 - After Close",
    "MSFT - Microsoft - ~Jul 29, 2026 - After Close",
    "META - Meta Platforms - ~Jul 29, 2026 - After Close",
    "AAPL - Apple - ~Jul 30, 2026 - After Close",
    "AMZN - Amazon - ~Jul 30, 2026 - After Close",
    "SNDK - SanDisk - ~Jul 30, 2026 - After Close",
]
...
def create_files_from_list() -> None:
    ...
    LOG.info(f"Pet list: {STOCKS_LIST}")

    for stock_name in STOCKS_LIST:
        filename: str = f"{stock_name}.txt"
        path: Path = PROCESSED_DIR / filename
        content: str = f"Pet data for: '{stock_name}'\n"
        write_text_file(path=path, content=content)
...
def create_files_using_list_comprehension() -> None:
    ...
    LOG.info(f"Original list: {STOCKS_LIST}")
    favorite_list: list[str] = [f"{prefix}{name}" for name in STOCKS_LIST]
...
def main() -> None:
    ...
    ##I only want it to call the files in the list, not the other types of loops
